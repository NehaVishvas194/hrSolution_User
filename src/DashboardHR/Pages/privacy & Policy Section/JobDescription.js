  ...
  const toWord = async (e, id, download_count) => {
    try {
      const response = axios.post(
        `${baseUrl}updateJobCountById/${id}`,
        { count: count + download_count },
        { responseType: "blob" }
      );
      getDataList();
      setRows((prevRows) =>
        prevRows.map((row) =>
          row._id === id ? { ...row, count: row.count + 1 } : row
        )
      );
    } catch (error) {
      console.error("Error downloading PDF:", error);
                  ...
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code}>
                    ...
                    <TableCell align="left">
                      ...
                      {row.jobWord ? (
                        <a
                          href={
                            "https://sisccltd.com/hrsolutions/" + row.jobWord
                          }
                          target="_blank"
                          rel="noopener noreferrer"
                          onClick={(e) => toWord(e, row._id, row.count)}
                        >
                          <ArticleIcon />
                        </a>
                      ) : (
              ...
              <Box>
                ...
                >
                  <>
                    ...
                    <p
                      dangerouslySetInnerHTML={renderHTML(responsibilities)}
                    ></p>
                  </>
