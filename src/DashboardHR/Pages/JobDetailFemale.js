...
export default function JobDetailFemale() {
    ...
    const handleFemaleJob = () => {
        axios.get(`${baseUrl}get_Female_jobseeker_profile/${getData.jobId}`)
            .then((response) => {
    ...
    const handlRetingUpdate = (e) => {
        ...
        axios.post(`${baseUrl}update_candidate_rating/${candidate}`, {
            ...
            if (response.status === 200) {
                ...
                setRows(prevRows =>
                    prevRows.map(row =>
                        row._id === candidate ? { ...row, candidate_rating: candidate_rating1 } : row
                    )
        ...
        <>
            ...
            <div className="container emp-profile" ref={targetRef}>
                ...
                <div className="ms-invoice-table table-responsive mt-4 px-3">
                    ...
                    <table className="table table-hover text-right thead-light" style={{ "white-space": "nowrap" }}>
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td>
                                            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                                                <InputLabel id={`demo-select-small-label-${i}`}>{row.jobSeeker_status === 2 ? "shortlist" : row.jobSeeker_status === 3 ? "longlisted" : row.jobSeeker_status === 4 ? "Assesment Schedule" : row.jobSeeker_status === 5 ? "Schedule_Interview" : null}</InputLabel>
                                        ...
                                        <td><EmailIcon onClick={(event) => handleClickOpen(event, row._id)} />/<WhatsAppIcon onClick={(event) => handleClickOpen2(event, row._id)} /></td>
