...
export default function Report() {
  ...
  const getActivedataList = () => {
    axios
      .get(
        `${baseUrl}/get_all_candidate_for_client/${localStorage.getItem(
          "empId"
        )}?gender=${data === "Female" ? data : age}`
      )
      .then((response) => {
  ...
  const dataObj = {
    ...
    gender: report,
    district: locations,
    job_type: selectedJobTitle,
  };
  const getReportData = () => {
    console.log(dataObj);
    axios
      .get(
        `${baseUrl}export_client_jobs_filteredcandidate/${localStorage.getItem(
          "empId"
        )}?from_date=${fromdate}&to_date=${todate}&gender=${report}&district=${locations}&job_type=${selectedJobTitle}`,
        {
    ...
    <div>
      <Paper sx={{ width: "100%", overflow: "hidden", padding: "12px" }}>
        ...
        <Stack direction="row" spacing={2} className="my-2 mb-2">
          ...
          <FormControl sx={{ width: 300, mt: 2 }} className="mt-2">
          ...
          {/* <Button variant="contained" size="large" >generate Report</Button> */}
        ...
        {rows.length === 0 ? (
          <>
            ...
            >
              Currently, No Candidates have applied for job
            </div>
            <Box
              component="section"
              sx={{ p: 2 }}
            ...
            <Table stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {rows
                  ...
                  .map((row, index) => (
                    <TableRow key={row._id} style={{ "white-space": "nowrap" }}>
                      ...
                      <TableCell align="left">{row.city || "_"}</TableCell>
                      <TableCell align="left">{row.job_Heading}</TableCell>
                      <TableCell align="left">
                        {row.candidateStatus === 0
                          ? "rejected"
                          : row.candidateStatus === 1
                          ? "pending"
                          : row.candidateStatus === 2
                          ? "screened"
                          : row.candidateStatus === 3
                          ? "complete"
                          : null}
                      </TableCell>
